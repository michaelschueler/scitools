module scitools_hermite
!! Contains some tools for constructing Hermite polynomials and functions
!======================================================================================
  implicit none
!--------------------------------------------------------------------------------------
   private 
   public :: h_integral, h_polynomial_coefficients, h_polynomial_value, &
    h_polynomial_values, h_polynomial_zeros, h_quadrature_rule, &
    he_integral, he_polynomial_coefficients, he_polynomial_value, &
    he_polynomial_values, he_polynomial_zeros, he_quadrature_rule, &
    hen_exponential_product, hen_polynomial_value, hen_power_product, &
    hf_exponential_product, hf_function_value, hf_function_values, &
    hf_power_product, hf_quadrature_rule, hn_exponential_product, &
    hn_polynomial_value, hn_power_product
...
!
!    Output, real ( kind = rk ) C(0:N,0:N), the coefficients of the polynomials
...
!
  ...
  do i = 2, n
    ...
    c(i,1:i-2) =   2.0D+00                            * c(i-1,0:i-3)  &
                  -2.0D+00 * real ( i - 1, kind = rk ) * c(i-2,1:i-2)
...
!
  ...
  do i = 2, n
    c(i,0)     =              - real ( i - 1, kind = rk ) * c(i-2,0)
    c(i,1:i-2) = c(i-1,0:i-3) - real ( i - 1, kind = rk ) * c(i-2,1:i-2)
    c(i,  i-1) = c(i-1,  i-2)
    c(i,  i  ) = c(i-1,  i-1)
  end do
...
!
  ...
  do j = 2, n
    p(1:m,j) = x(1:m) * p(1:m,j-1) - real ( j - 1, kind = rk ) * p(1:m,j-2)
  end do
...
!
!    Output, real ( kind = rk ) HE_TRIPLE_PRODUCT_INTEGRAL, the value 
!    of the integral.
!
  implicit none
...
!  Parameters:
!
!    Input, integer P, the maximum degree of the 
...
!
  ...
  do k = 1, order
    ...
    x(1) = x_table(k);
    call hen_polynomial_value ( 1, p, x(1), h_table )
!
...
!    Input, real ( kind = rk ) X(M), the evaluation points.
!
!    Output, real ( kind = rk ) P(M,0:N), the values of the polynomials of 
!    index 0 through N.
!
  implicit none
...
!
  ...
  do k = 1, order
    ...
    x(1) = x_table(k)
    call hf_function_value ( 1, p, x(1), h_table )
!
!  The following formula is an outer product in H_TABLE.
!
    do j = 0, p
...
!    Input, real ( kind = rk ) X(M), the evaluation points.
!
!    Output, real ( kind = rk ) F(M,0:N), the values of the Hermite functions 
...
!
  ...
  do j = 2, n
    f(1:m,j) = ( sqrt ( 2.0D+00 ) * x(1:m) * f(1:m,j-1) &
      - sqrt ( real ( j - 1, kind = rk ) ) * f(1:m,j-2) ) &
      / sqrt ( real ( j, kind = rk ) )
  end do
...
!
  ...
  do k = 1, order
    ...
    x(1) = x_table(k)
    call hn_polynomial_value ( 1, p, x(1), h_table )
!
!  The following formula is an outer product in H_TABLE.
!
    do j = 0, p
...
subroutine monomial_to_h_matrix ( n, A )
...
!
  ...
  do j = 3, n
    do i = 1, n
      if ( i == 1 ) then
        A(i,j) = ( ( j - 2 ) * A(i,j-2)              ) / 2.0
      else
        A(i,j) = ( ( j - 2 ) * A(i,j-2) + A(i-1,j-1) ) / 2.0
      end if
    end do
  end do
