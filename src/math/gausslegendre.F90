...
!  * Jacob Williams
    ...
    real(dp),parameter :: one_half  = 0.5_dp
    ...
    type,extends(integration_class),public :: integration_class_1d
        ...
        real(dp) :: a      = zero      !! lower limit of integration
        real(dp) :: b      = zero      !! upper limit of integration (may be less than a)
        real(dp) :: tol    = zero      !! the requested relative error tolerance.

        real(dp) :: val = zero   !! the value of `x`. Only used for multiple
    ...
    abstract interface
        ...
        function func_1d(me,x) result(f)
            ...
            import :: dp,integration_class_1d
            implicit none
        ...
        function func_2d(me,x,y) result(f)
            ...
            import :: dp,integration_class_2d
            implicit none
        ...
        function func_3d(me,x,y,z) result(f)
            ...
            import :: dp,integration_class_3d
            implicit none
        ...
        function func_4d(me,x,y,z,q) result(f)
            ...
            import :: dp,integration_class_4d
            implicit none
        ...
        function func_5d(me,x,y,z,q,r) result(f)
            ...
            import :: dp,integration_class_5d
            implicit none
        ...
        function func_6d(me,x,y,z,q,r,s) result(f)
            ...
            import :: dp,integration_class_6d
            implicit none
...
!********************************************************************************
    ...
    real(dp),intent(in)   :: xl       !! x integration lower bound
    real(dp),intent(in)   :: xu       !! x integration upper bound
    real(dp),intent(in)   :: tolx     !! error tolerance for dx integration
...
!!  Must be called before integration is performed.
    ...
    real(dp),intent(in)    :: xl       !! x integration lower bound
    real(dp),intent(in)    :: xu       !! x integration upper bound
    real(dp),intent(in)    :: yl       !! y integration lower bound
    real(dp),intent(in)    :: yu       !! y integration upper bound
    real(dp),intent(in)    :: tolx     !! error tolerance for dx integration
    real(dp),intent(in)    :: toly     !! error tolerance for dy integration
    integer,intent(in)     :: methodx  !! quadrature method to use for x
...
!!  Must be called before integration is performed.
    ...
    real(dp),intent(in)    :: yl       !! y integration lower bound
    real(dp),intent(in)    :: yu       !! y integration upper bound
    real(dp),intent(in)    :: zl       !! z integration lower bound
    real(dp),intent(in)    :: zu       !! z integration upper bound
    real(dp),intent(in)    :: tolx     !! error tolerance for dx integration
    real(dp),intent(in)    :: toly     !! error tolerance for dy integration
    real(dp),intent(in)    :: tolz     !! error tolerance for dz integration
    integer,intent(in)     :: methodx  !! quadrature method to use for x
    integer,intent(in)     :: methody  !! quadrature method to use for y
...
!!  Must be called before integration is performed.
    ...
    real(dp),intent(in)    :: zl       !! z integration lower bound
    real(dp),intent(in)    :: zu       !! z integration upper bound
    real(dp),intent(in)    :: ql       !! q integration lower bound
    real(dp),intent(in)    :: qu       !! q integration upper bound
    real(dp),intent(in)    :: tolx     !! error tolerance for dx integration
    real(dp),intent(in)    :: toly     !! error tolerance for dy integration
    real(dp),intent(in)    :: tolz     !! error tolerance for dz integration
    real(dp),intent(in)    :: tolq     !! error tolerance for dq integration
    integer,intent(in)     :: methodx  !! quadrature method to use for x
    integer,intent(in)     :: methody  !! quadrature method to use for y
    integer,intent(in)     :: methodz  !! quadrature method to use for z
...
!!  Must be called before integration is performed.
    ...
    real(dp),intent(in)    :: ql       !! q integration lower bound
    real(dp),intent(in)    :: qu       !! q integration upper bound
    real(dp),intent(in)    :: rl       !! r integration lower bound
    real(dp),intent(in)    :: ru       !! r integration upper bound
    real(dp),intent(in)    :: tolx     !! error tolerance for dx integration
    real(dp),intent(in)    :: toly     !! error tolerance for dy integration
    real(dp),intent(in)    :: tolz     !! error tolerance for dz integration
    real(dp),intent(in)    :: tolq     !! error tolerance for dq integration
    real(dp),intent(in)    :: tolr     !! error tolerance for dr integration
    integer,intent(in)     :: methodx  !! quadrature method to use for x
    integer,intent(in)     :: methody  !! quadrature method to use for y
    integer,intent(in)     :: methodz  !! quadrature method to use for z
    integer,intent(in)     :: methodq  !! quadrature method to use for q
...
!!  Must be called before integration is performed.
    ...
    real(dp),intent(in)    :: rl       !! r integration lower bound
    real(dp),intent(in)    :: ru       !! r integration upper bound
    real(dp),intent(in)    :: sl       !! s integration lower bound
    real(dp),intent(in)    :: su       !! s integration upper bound
    real(dp),intent(in)    :: tolx     !! error tolerance for dx integration
    real(dp),intent(in)    :: toly     !! error tolerance for dy integration
    real(dp),intent(in)    :: tolz     !! error tolerance for dz integration
    real(dp),intent(in)    :: tolq     !! error tolerance for dq integration
    real(dp),intent(in)    :: tolr     !! error tolerance for dr integration
    real(dp),intent(in)    :: tols     !! error tolerance for ds integration
    integer,intent(in)     :: methodx  !! quadrature method to use for x
    integer,intent(in)     :: methody  !! quadrature method to use for y
    integer,intent(in)     :: methodz  !! quadrature method to use for z
    integer,intent(in)     :: methodq  !! quadrature method to use for q
...
!!  See also: Original sourcecode from: http://www.netlib.org/slatec/src/dgaus8.f
    ...
    class(integration_class_1d),intent(inout)  :: me
    real(dp),intent(in)   :: lb         !! lower bound of the integration
    real(dp),intent(in)   :: ub         !! upper bound of the integration
    real(dp),intent(in)   :: error_tol  !! is a requested pseudorelative error tolerance.  normally
                                        !! pick a value of abs(error_tol) so that
                                        !! dtol < abs(error_tol) <= 1.0e-3 where dtol is the larger
                                        !! of 1.0e-18 and the real(dp) unit roundoff d1mach(4).
    ...
    real(dp),intent(out)  :: ans        !! computed value of integral
    ...
    real(dp),intent(out)  :: err        !! an estimate of the absolute error in `ans`.
    ...
    real(dp),parameter  :: bb       = radix(one)          !! machine constant
    real(dp),parameter  :: d1mach4  = bb**(1-digits(one)) !! machine constant
    real(dp),parameter  :: d1mach5  = log10(bb)           !! machine constant

    integer                   :: k,l,lmn,lmx,mxl,nbits,nib,nlmx
    real(dp)                  :: ae,anib,area,c,ee,ef,eps,est,gl,glr,tol
    real(dp),dimension(iwork) :: aa,hh,vl,gr
    ...
    nbits = anib
    nlmx = min(100,(nbits*5)/8)         ! ... is this the same 60 as iwork???
    lmx = nlmx
    lmn = nlmn
    if (ub /= zero) then
...
!!    http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
    ...
    real(dp)                                :: f
     ...
     abscissae:
    real(dp),dimension(3),parameter ::  a = [   0.6612093864662645136613995950199053470064485643&
                                                ...
                                                &771150514997832_dp ]
     weights:
    real(dp),dimension(3),parameter ::  w = [   0.36076157304813860756983351383771611166152189274&
...
!!      http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
    ...
    real(dp)             :: f
     ...
     abscissae:
    real(dp),parameter ::   x1 = 0.18343464249564980493947614236018398066675781291297378231718847&
                                                ...
                                                &34052985058171969174306064445289638150514997832_dp
    real(dp),parameter ::   x2 = 0.52553240991632898581773904918924634904196424312039285775085709&
                                                ...
                                                &88817537938539658502113876953598879150514997832_dp
    real(dp),parameter ::   x3 = 0.79666647741362673959155393647583043683717173161596483207017029&
                                                ...
                                                &36113628137267630651413319993338002150514997832_dp
    real(dp),parameter ::   x4 = 0.96028985649753623168356086856947299042823523430145203827163977&
                                                ...
                                                &8923208763998895240977248938732425615051499783203_dp

     weights:
    real(dp),parameter ::   w1 = 0.36268378337836198296515044927719561219414603989433054052482306&
                                                ...
                                                &28976560993309027632737537826127502150514997832_dp
    real(dp),parameter ::   w2 = 0.31370664587788728733796220198660131326032899900273493769026394&
                                                ...
                                                &51736972560423953923732838789657919150514997832_dp
    real(dp),parameter ::   w3 = 0.22238103445337447054435599442624088443013087005124956472590928&
                                                ...
                                                &52819880671616779621183704306688233150514997832_dp
    real(dp),parameter ::   w4 = 0.10122853629037625915253135430996219011539409105168495705900369&
                                                ...
                                                &66466585774650346143712142008566866150514997832_dp
...
!!    http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
    ...
    real(dp)             :: f
     ...
     abscissae:
    real(dp),dimension(5),parameter ::  a = [   0.14887433898163121088482600112971998461756485942&
                                                ...
                                                &76788807708483879452488118549797039287926964254222_dp,&
                                                ...
                                                &80274268486617121011712030227181051010718804444161_dp,&
                                                ...
                                                &36942765911483643714816923781701572597289139322313_dp,&
                                                ...
                                                &88311646653581707125686970668737259229449284383797_dp ]

     weights:
    real(dp),dimension(5),parameter ::  w = [   0.29552422471475287017389299465133832942104671702&
                                                ...
                                                &0775158914556548308329986393605934912382356670244_dp,&
                                                ...
                                                &86749842814086288868533208042150419508881916391898_dp,&
                                                ...
                                                &85349420711613410227291565477768928313300688702802_dp,&
                                                ...
                                                &27785361756927642128792412282970150172590842897331_dp,&
                                                ...
                                                &9236378557180717569208295026105115288152794421677_dp ]
...
!!    http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
    ...
    real(dp)             :: f
     ...
     abscissae:
    real(dp),dimension(6),parameter ::  a = [   0.12523340851146891547244136946385312998339691630&
                                                ...
                                                &5097342064247739686883799517760948964137522919201_dp,&
                                                ...
                                                &62319787423676668046033025242558536362617894366679_dp,&
                                                ...
                                                &96877940491139756923177447689738849120865435563147_dp,&
                                                ...
                                                &10899341797549230707021382467596975621464477134163_dp,&
                                                ...
                                                &64065954540171679180850205196702894963912359448494_dp,&
                                                ...
                                                &9383170419389543470920618543180673569225988370568_dp]

     weights:
    real(dp),dimension(6),parameter ::  w = [   0.24914704581340278500056243604295121083046090256&
                                                ...
                                                &29347837170742904111565650371846972323325015720931_dp,&
                                                ...
                                                &15375809270840669989018891281956753131165193423269_dp,&
                                                ...
                                                &2327927963339557545261423500783899286052850767594_dp,&
                                                ...
                                                &70458396283884610940466728874776625823124924247387_dp,&
                                                ...
                                                &98358517298903536137451828089012822811396588037254_dp,&
                                                ...
                                                &63180160342232703368882666374567833050364187887189_dp]
...
!!    http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
    ...
    real(dp)             :: f
     ...
     abscissae:
    real(dp),dimension(7),parameter ::  a = [  0.10805494870734366206624465021983474761195160547&
                                               ...
                                               &2506161702814425014635643221773541001328892761_dp,&
                                               ...
                                               &2949997448460558214759676525644841351801594858_dp,&
                                               ...
                                               &3979637438627635490786064892912451481973721288_dp,&
                                               ...
                                               &7877996475717650653147982694804026342351254071_dp,&
                                               ...
                                               &6425172143208924251106624044295037127737490111_dp,&
                                               ...
                                               &5749388483890945436457404705549484348178721002_dp,&
                                               ...
                                               &3676463914802477677291745002965827767360741735_dp ]

     weights:
    real(dp),dimension(7),parameter ::  w = [   0.21526385346315779019587644331626003527499755805&
                                               ...
                                               &487266562730897512393716549425155133887783267_dp,&
                                               ...
                                               &8072394707049964721031773292256965337005468577_dp,&
                                               ...
                                               &6150274514935557012909889503067356410067833406_dp,&
                                               ...
                                               &9084685022864905124096570215866733146092008329_dp,&
                                               ...
                                               &1072357341821083329663867464821867539341968434_dp,&
                                               ...
                                               &0937535198838649279475628473516378736712929573_dp,&
                                               ...
                                               &9810087587180865408885105556219147609526200925_dp ]
...
!*******************************************************************************************************
    end module scitools_gausslegendre
!*******************************************************************************************************